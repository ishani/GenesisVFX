...
{	public :
			...
			uchar	ClipMode;						// how to occlude with the mask
			...
			HWND			Status;
//			void			SetupStatus(HWND hWnd)	{ if (Status) { Status=hWnd;
//																  }
//													}

			void			StatusText(char *F)		{ 
														char Tmp[256];
...
//													}
			...
			BOOL			Abort(void)				{ return AbortIt();
													}
...
class Flare
	{ private:
		...
		//***********************************************************************************************************
		//***********************************************************************************************************
		float lerp(float *In,float a) { int pos1=(int)floor(a),pos2=pos1+1;
		...
		void lerp(fColour *In,float a,fColour &fin) 
									  { int pos1=(int)floor(a),pos2=pos1+1;
		...
		float lerpwrap(float *In,float a) { int pos1=(int)floor(a),pos2=pos1+1;
		                                    if (pos2>=FLARERESOLUTION) pos2=0;
...
#undef Tween
		...
		float			w2f(WORD a) { return ((float)a)*1.52590218967E-5f; }
		...
		public: 
			...
			void RenderPreview(char *Bitmap,int xres,int yres,int Checker,float Size,float Time,float Sample)
						{	RenderNFO rinfo; rinfo.Init();
							...
							rinfo.UseSingle=1;
							rinfo.xpos=xres/2.0f; 
							rinfo.ypos=yres/2.0f;
							...
							if (Light>5) { Alloc2D(rinfo.Bak_R,uchar,yres,xres);
							...
							// Free Memory
							if (Light>5) { Free2D(rinfo.Bak_R,lyres);
