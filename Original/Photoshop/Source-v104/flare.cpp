...
BOOL Flare::PreProcess(int xres, int yres, float xpos, float ypos, float CameraDepth,float ir,float ig,float ib,float ia,float Time,float RDepth,float Size)
			  ...
			  // Get Centre And Reder Position
					RenderCX=TrueX*dx1+TrueY*dx2;		// These Are Always The Same In This Version
			  ...
			  // Angle Offsets
					...
					AngleAdd=(float)(-0.5f*AngleOffSet*FLARERESOLUTION);
					if (ReAlign) AngleAdd-=(float)FLARERESOLUTION*(float)atan2(RenderCY-ScreenCY,RenderCX-ScreenCX)/6.28319530718f;
					while(AngleAdd<0) AngleAdd+=FLARERESOLUTION;

			  // Compute Bounding Box
					if (Inferno.IsUsed())
							{ MaxX=xres; MaxY=yres;		//uugghh slow :((
							  MinY=0; MinX=0;
							}
					else
							{ MaxX=(int)(RenderCX+(float)(RenderSize*xres)+1);
							  MinX=(int)(RenderCX-(float)(RenderSize*xres)-1);
							  MaxY=(int)(RenderCY+(float)(RenderSize*xres)+1);
							  MinY=(int)(RenderCY-(float)(RenderSize*xres)-1);
							}

			  // FractalNoise Stuff
					Inferno.Setup(RenderSize*xres,RenderCX,RenderCY,Time); // TODO : Time
...
void Flare::Blur(float &Screen,uchar **Back,float _x,float _y,int xres,int yres,float Width)
	{	float	Sigma=(float)-2.302/(Width*Width);					// Sigma Squared is more useful
		...
		int lxmax=(int)_x+Range; if (lxmax>=xres) lxmax=xres-1;
		int lymax=(int)_y+Range; if (lymax>=yres) lymax=yres-1;
		int lxmin=(int)_x-Range; if (lxmin<0)     lxmin=0;
		int lymin=(int)_y-Range; if (lymin<0)     lymin=0;
		...
		for(yy=(int)_y,y=0;yy<=lymax;yy++,y++)
		for(xx=(int)_x,x=0;xx<=lxmax;xx++,x++)
		...
		if (x>0)
			for(yy=(int)_y  ,y=0;yy<=lymax;yy++,y++)
			for(xx=(int)_x-1,x=1;xx>=lxmin;xx--,x++)
					{ Screen+=i2f(Back[yy][xx])*Tmp[y*Range+x];
					  Normalise+=Tmp[y*Range+x];
					}

		if (y>0)
			for(yy=(int)_y-1,y=1;yy>=lymin;yy-=1,y+=1)
			for(xx=(int)_x  ,x=0;xx<=lxmax;xx++,x++)
					{ Screen+=i2f(Back[yy][xx])*Tmp[y*Range+x];
					  Normalise+=Tmp[y*Range+x];
					}
		...
		if ((x>0)&&(y>0))
			for(yy=(int)_y-1,y=1;yy>=lymin;yy--,y++)
			for(xx=(int)_x-1,x=1;xx>=lxmin;xx--,x++)
					{ Screen+=i2f(Back[yy][xx])*Tmp[y*Range+x];
					  Normalise+=Tmp[y*Range+x];
					}
...
// rcw can't use mbuffer - holds mask
...
//												}
						  ...
						  if( rinfo.ClipMode==2 ) mval=0.0f;
						  else if( rinfo.ClipMode==1 ) mval=1.0f-rinfo.mbuffer[Posn]/255.0f;
						  else mval=rinfo.mbuffer[Posn]/255.0f;
						  float _tmp=(1.0f-Occlusion)*mval; // should be mask val *i2f(Img_A[y][x]);		// Better Aliasing
			...
			for(x=StartX;x<=EndX;x++)
			  if (rinfo.AntiAliasing==1.0f)
							{ float _r=i2f(Img_R[y][x]);
							  ...
							  tmp=GetColour2(_r,_g,_b,_a,Bak_R,Bak_G,Bak_B,Bak_A,(float)x,(float)y,rinfo.xres,rinfo.yres);
							  ...
							  for(float sy=(float)y;sy<(float)y+1.0f;sy+=rinfo.AntiAliasing,n++)
									{ float _r=i2f(Img_R[y][x]);
									  ...
									  tmp|=GetColour(_r,_g,_b,_a,Bak_R,Bak_G,Bak_B,Bak_A,(float)x,(float)y,rinfo.xres,rinfo.yres);
